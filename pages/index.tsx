...
const Home: NextPage = () => {
  ...
  const checkOrClaimReady = (): boolean => {
    return userAddrNois !== '' && validateAddressestwo(userAddrJuno) !== undefined
        ...
        <div className="row-span-5 pt-20 flex flex-col gap-y-8 justify-center items-center bgx-nois-blue">
          <div className="flex gap-x-4 w-2/3 justify-center items-center">
            <input
              ...
              placeholder='nois1...  /  juno1...  /  stars1... /  inj1...  /  aura1...'
          ...
          <div className="flex flex-col gap-y-2 w-5/6 justify-center items-center borderx">
            <div className="flex gap-x-2 wx-full w-3/5">
              ...
              <div className="flex flex-col wx-1/2 w-full gap-y-1">
                <div className="p-2 flex justify-start gap-x-4 text-nois-white border border-nois-white/30 bg-slate-600/10">
                  <span className="text-nois-white/30">
                    {"Juno Address:"}
                  </span>
                  {userAddrJuno.length > 3 ?
                    userAddrJuno :
                    <div className="text-nois-white/50">Connect wallet or Enter address above and click Submit</div>
                  }
                </div>
                <div 
                  className={`${junoAmt === undefined ? "hidden" : "flex"} ${junoAmt === "0" ? "text-red-400" : "text-green-500"} text-base w-full justify-center`}>
                  {junoAmt ? junoAmt === "0" ? "None" : `33% chance of: ${fromMicro(Number(junoAmt) * 3)} NOIS` : ""}
                </div>
              </div>
            </div>
          ...
          <div className="flex justify-center gap-x-4 text-xl ">
            {checkOrClaimReady() === true ? (
              <>
                <button
                  className={`${!userAddrJuno || validateAddresses(userAddrJuno) === undefined ? "opacity-50 border border-white/50" : "shadow-neon-md animate-pulse border border-[#6ff2cf] text-[#6ff2cf] hover:bg-[#6ff2cf30]"} px-4 py-2 rounded-lg`}
                  onClick={() => {
                    if (loading !== true) {
                      if (!batchClient) {
